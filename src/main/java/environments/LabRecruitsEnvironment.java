...
/**
 * This "Environment" provides methods to control and get observations from a
 * running instance of the Lab Recruits game. To programmatically launch an
 * instance of Lab Recruits from Java you can use
 * {@link game.LabRecruitsTestServer}.
 * 
 * To create an instance of this LabRecruitsEnvironment, you also need to
 * specify the {@link LabRecruitsConfig} that is to be used. Most importantly,
 * this configuration specifies which level is to be load.
 * 
 * The typical way to launch Lab Recruits and hook an environment is e.g.:
 * 
 * <pre>
 * var game = new LabRecruitsTestServer(true, path - to - labrecruits - executable);
 * game.waitForGameToLoad();
 * var config = new LabRecruitsConfig(levelname, path - to - dir - of - level - defs);
 * LabRecruitsEnvironment env = new LabRecruitsEnvironment(config);
 * </pre>
 * 
 * From this point on you can invoke env's methods to control the game.
 */
public class LabRecruitsEnvironment extends W3DEnvironment {
    ...
    /**
     * Create an instance of this environment with the standard Lab Recruits
     * configuration.
     */
    ...
    /**
     * Create an instance of this environment with the given Lab Recruits
     * configuration.
     */
    ...
    /**
     * Instruct the Lab Recruits game to load the level as specified in
     * gameConfig(). After the level is loaded, the game will send back the
     * navigation-mesh of the level. This will be stored in the field
     * worldNavigableMesh. This mesh is static (does not change).
     */
    @Override
    public void loadWorld() {
        var rawmesh = (LabRecruitsRawNavMesh) sendCommand(null, null, LOADWORLD_CMDNAME, null,
                LabRecruitsRawNavMesh.class);
        if (rawmesh == null)
    ...
    /**
     * Return what the specified agent currently observes. This is represented as a
     * WorldModel structure. E.g. this information includes the agent's own position
     * amd health, game objects it sees, their positions, and their properties.
     */
    @Override
    ...
    /**
     * Will move the specified agent towards the given target location. This does
     * not necessarily bring the agent to that destination location. The agent will
     * move in that direction, as far as its speed. The default speed is 0.13.
     * Furthermore, this allows there is no obstacle obstructs the movement.
     * 
    ...
    /**
     * This causes the agent to interacts with a game-object specified by its id.
     * Note that to interact, the agent needs to be close enough (<= 1.5) to the
    ...
    /**
     * Unpaused Lab Recruits, if it was paused.
     */
    ...
    /**
     * this function updates the hazards in Unity, which is specified in
     * EnvironmentConfig. Currently broken :| TODO.
     */
    public Boolean updateHazards() {
        throw new UnsupportedOperationException();
        /*
         * try { return sendPackage(Request.updateEnvironment()); } catch (IOException
         * e) { System.out.println(e.getMessage()); System.out.println(String.
         * format("%s: Sending an update-environment command fails.",
         * PrintColor.FAILURE())); return false; }
         */
    }
    ...
    public boolean close() {
        try {
            ...
            if (success) {
                ...
                System.out.println(
                        String.format("%s: Unity does not respond to a disconnection request.", PrintColor.FAILURE()));
            }
            return success;
        } catch (IOException e) {
            System.out.println(e.getMessage());
            System.out.println(String.format("%s: Could not disconnect from the host by closing the socket.",
                    PrintColor.FAILURE()));
            return false;
        }
    }
    ...
    /**
     * Primitive for sending a command-package to Lab-Recruit, and to return its
     * response. The command to send should be wrapped as a "Request" object.
     */
