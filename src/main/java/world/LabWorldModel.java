...
public class LabWorldModel extends WorldModel {
    ...
    /**
     * Describing the part of the static world that the agent currently sees. Here,
     * it is described as a set of nodes in the world's navigation graph that the
     * agent sees. Each node is represented by a single integer which is an index to
     * where the node is stored in the world's navigation graph. The graph itself is
     * not represented in this object; though it is assumed that the agent has a way
     * to access it.
     */
    ...
    /**
     * Return the center position of the agent, with the y-position shifted to the
     * floor level.
     */
    ...
    /**
     * The agent can interact with e is e is a switch, and the agent is at e's
     * location.
     */
    @Override
    public boolean canInteract(String interactionType, WorldEntity e) {
        ...
        if (target.type != LabEntity.SWITCH)
            return false;
        ...
        // LR's interaction distance threshold is set at 1.5, so its squared-value is
        // 2.25,
    ...
    public boolean isBlocking(WorldEntity e) {
        switch (e.type) {
        case LabEntity.DOOR:
            return !e.getBooleanProperty("isOpen");
        case LabEntity.COLORSCREEN:
            return true;
        case LabEntity.GOAL:
            return true;
        default:
    ...
    public LabWorldModel interact(W3DEnvironment env, String interactionType, WorldEntity e) {
        ...
        var target = (LabEntity) e;
        // using != should be ok in this case, as they should point to the same string
        // as well:
        if (target.type != LabEntity.SWITCH)
            return null;
