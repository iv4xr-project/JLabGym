...
public class LabEntity extends WorldEntity implements LineIntersectable {
    ...
    /**
     * Return the center position of the entity, with the y-position shifted to the
     * floor level.
     */
    ...
    public boolean hasSameState(WorldEntity old) {
        ...
        switch (this.type) {
        case DOOR:
            return getBooleanProperty("isOpen") == old.getBooleanProperty("isOpen");
        case SWITCH:
            return getBooleanProperty("isOn") == old.getBooleanProperty("isOn");
        case COLORSCREEN:
            return getProperty("color").equals(old.getProperty("color"));
        }
        return false;
    }
    ...
    public Collection<Vec3> intersect(Line l) {
        ...
        if (type.equals(DOOR) || type.equals(COLORSCREEN)) {
            // use a box to calculate the intersection with this door :D .. stretch the
            // extent a bit larger
